...
config_xray() {
    ...
    # The username and password are no longer required for SOCKS configuration.
    # The script now assumes no authentication (username/password is blank).
    ...
    for ((i = 0; i < ${#IP_ADDRESSES[@]}; i++)); do
        ...
        if [ "$config_type" == "socks" ]; then
            # For a SOCKS server with no authentication, we avoid setting accounts.
